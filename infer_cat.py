...
checkpoint_path = "/home/usuaris/veu/daniel.gonzalbez/logs/vits_festcat-June-09-2023_04+37PM-0000000/checkpoint_140000.pth"
...
text = "Vaig a menjar un tros de pa abans d'anar a passejar pels carrers del voltant. Quin gran dia fa! No puc tenir més goig."
...
sample = {"text": text, "language": 'ca', "audio_unique_name": "catalannnn"}
...
spk_emb = torch.load(root_path + '/festcat/f1_ref2/' + spk_emb_dic['f1_ref']).squeeze(-1)
outputs = model.inference(tokens, aux_input = {"d_vectors":spk_emb})
torchaudio.save("results/cat_f1140.wav", outputs["model_outputs"].squeeze().unsqueeze(0), 16000)

spk_emb = torch.load(root_path + '/festcat/m4_ref2/' + spk_emb_dic['m4_ref']).squeeze(-1)
outputs = model.inference(tokens, aux_input = {"d_vectors":spk_emb})
torchaudio.save("results/cat_m4140.wav", outputs["model_outputs"].squeeze().unsqueeze(0), 16000)

spk_emb = torch.load(root_path + '/festcat/m5_ref2/' + spk_emb_dic['m5_ref']).squeeze(-1)
outputs = model.inference(tokens, aux_input = {"d_vectors":spk_emb})
torchaudio.save("results/cat_m5140.wav", outputs["model_outputs"].squeeze().unsqueeze(0), 16000)
