...
checkpoint_path = "/home/usuaris/veu/daniel.gonzalbez/logs/vits_tcstar-June-13-2023_04+00PM-0000000/checkpoint_120000.pth"
...
for i, text in enumerate(text_es):
    ...
    os.chdir('/home/usuaris/veu/daniel.gonzalbez/test_ref_norm')

    for file in os.listdir():
        if file.endswith('.pt') and 'dani' not in file and 'guille' not in file:
...
os.chdir('/home/usuaris/veu/daniel.gonzalbez')
...
# known speakers:
for i, text in enumerate(text_cat):
    sample = {"text": text, "language": 'ca', "audio_unique_name": "text_CAT"+str(i)}
    ...
    for file in os.listdir():
        if file.endswith('.pt'):
            spk_emb = torch.load(file).squeeze(-1)
            outputs = model.inference(tokens, aux_input = {"d_vectors":spk_emb, "language_ids":torch.tensor([1])})
            torchaudio.save("/home/usuaris/veu/daniel.gonzalbez/test_syn/" + file.replace('_lstm.pt', '_alpha6_cat' + str(i)+ '.wav'), outputs["model_outputs"].squeeze().unsqueeze(0), 16000)
...
checkpoint_path = "/home/usuaris/veu/daniel.gonzalbez/logs/vits_tcstar-June-08-2023_06+32PM-0000000/checkpoint_110000.pth"
root_path = "/home/usuaris/veu/daniel.gonzalbez"
...
for i, text in enumerate(text_es):
    sample = {"text": text, "language": 'es', "audio_unique_name": "text_ESP"+str(i)}
    ...
    for file in os.listdir():
        if file.endswith('.pt'):
            spk_emb = torch.load(file).squeeze(-1)
            outputs = model.inference(tokens, aux_input = {"d_vectors":spk_emb, "language_ids":torch.tensor([0])})
            torchaudio.save("/home/usuaris/veu/daniel.gonzalbez/test_syn/" + file.replace('_lstm.pt', '_multi_es' + str(i)+ '.wav'), outputs["model_outputs"].squeeze().unsqueeze(0), 16000)
...
checkpoint_path = "/home/usuaris/veu/daniel.gonzalbez/logs/vits_tcstar-June-06-2023_07+57PM-0000000/checkpoint_110000.pth"
root_path = "/home/usuaris/veu/daniel.gonzalbez"
...
for i, text in enumerate(text_cat):
    ...
    for file in os.listdir():
        if file.endswith('.pt'):
            spk_emb = torch.load(file).squeeze(-1)
            outputs = model.inference(tokens, aux_input = {"d_vectors":spk_emb, "language_ids":torch.tensor([1])})
            torchaudio.save("/home/usuaris/veu/daniel.gonzalbez/test_syn/" + file.replace('_lstm.pt', '_multi_cat' + str(i)+ '.wav'), outputs["model_outputs"].squeeze().unsqueeze(0), 16000)
"""